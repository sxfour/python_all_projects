...
class OPCResponse:
    ...
    def get_data_opc(self, sleep_conn):
        while True:
            table = Table(
                "Полный тег", "Тег", "Значение", "Время", box=box.MINIMAL, title=self.title
            )
                ...
                while True:
                    try:
                        ...
                        for value_node in self.device:
                            ...
                            table.add_row(str(var.partition(".")[2]), str(value_node), str(round(node, 4)), f_time)
...
if __name__ == "__main__":
    ...
    with Live(console=console, screen=True, auto_refresh=False) as live:
        while True:
            try:
                live.update(OPCResponse("", device=TSRV_022).get_data_opc(sleep_conn=1), refresh=True)
