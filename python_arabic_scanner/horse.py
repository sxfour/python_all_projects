...
def basic_scan(port, host):
    ...
    # If you need to faster, set 0.5 seconds timeout sockets, smb connections
    sock.settimeout(0.5)
    try:
...
def network_map(source, weights):
    ...
    net = Network(height='625px', width='625px', bgcolor='#222222', font_color='white', filter_menu=True)
    net.barnes_hut()
    ...
    print(f'{green}[+]{end} Saved from: AllArabic(Linux)/main/test.html')


if __name__ == '__main__':
    ips, ipe = '78.200.41.186-0.0.0.0'.split('-')
    ipr = IPRange(ips, ipe)
    port = 445
    for ip in ipr:
