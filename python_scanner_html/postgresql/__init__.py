...
class AddHosts:
    ...
    def appendHTTPHostPostgreSQL(self, db, host, port, header_info, png_path):
        self.addPostgreSQL("INSERT INTO public.\"{0}\" (ip_address, port_opened, header_info, png_file) VALUES "
                           "('{1}', '{2}', '{3}', '{4}'); commit;"
                           .format(db, host, port, header_info, Binary(png_path)))
    ...
    def addPostgreSQLBinary(self, request):
        return WorkerSQL(request, self.settings, self.section).addPostgreSQL()
